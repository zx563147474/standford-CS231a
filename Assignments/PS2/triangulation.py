...
def linear_estimate_3d_point(image_points, camera_matrices):
    ...
    '''
    p_num = image_points.shape[0]
    A = np.zeros((2*p_num, 4))
...
def jacobian(point_3d, camera_matrices):
    ...
    for i in range(M):
        m = camera_matrices[i]
        y1 = m[2,3] + P1 * m[2,0] + P2 * m[2,1] + P3 * m[2,2]
        y2 = m[1,3] + P1 * m[1,0] + P2 * m[1,1] + P3 * m[1,2]
        y3 = m[0,3] + P1 * m[0,0] + P2 * m[0,1] + P3 * m[0,2]
        jacobian[i*2, 0] = m[0,0]/y1 - m[2,0]*y3/y1**2
        jacobian[i*2, 1] = m[0,1]/y1 - m[2,1]*y3/y1**2
        jacobian[i*2, 2] = m[0,2]/y1 - m[2,2]*y3/y1**2
        jacobian[i*2 + 1, 0] = m[1,0]/y1 - m[2,0]*y2/y1**2
        jacobian[i*2 + 1, 1] = m[1,1]/y1 - m[2,1]*y2/y1**2
        jacobian[i*2 + 1, 2] = m[1,2]/y1 - m[2,2]*y2/y1**2
    return jacobian
...
if __name__ == '__main__':
    ...
    print('-' * 80)
    print('Part B: Check that the difference from expected point is near zero')
    print('-' * 80)
    ...
    print('expected_3d_point:', expected_3d_point)
    print('estimated_3d_point:', estimated_3d_point)
    ...
    print('-' * 80)
    print('Part C: Check that the difference from expected error/Jacobian is near zero')
    print('-' * 80)
